...
{{--                    <td>{{ $paciente -> dob }}</td>--}}
                    ...
                    <td>
                        @if($paciente -> action_last == '1') Interv
                        @elseif($paciente -> action_last == '2') Hosp
                        @elseif($paciente -> action_last == '3') Amb
                        @elseif($paciente -> action_last == '4') Otra
                        @endif
                        @foreach($action_names as $action_name)
                            @if($action_name->id == $paciente->action_last)
                                {{ $action_name->shortName }}
                            @endif
                        @endforeach
                    </td>
{{--                    <td>{{ $paciente -> action_last_date }}</td>--}}
{{--                    <td>{{ \Carbon\Carbon::parse($paciente -> action_last_date)->format('d-m-Y') }}</td>--}}
                    <td>
                        @if(isset($paciente -> action_last_date))
{{--                            {{ \Carbon\Carbon::parse($paciente -> action_last_date)->format('d-m-Y') }}--}}
                            {{ \Carbon\Carbon::parse($paciente -> action_last_date)->format('Y-m-d') }}
                        @endif
                    </td>
    ...
    {{--                        <a href="{{ route('paciente.edit', [$paciente->id, 'origen'=>'index']) }}" class="btn btn-sm btn-primary">Editar</a>--}}
                            <a href="{{ route('paciente.edit', [$paciente->id, 'origen'=>'index_alta']) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar"><i class="fa fa-lg fa-fw fa-pen"></i></a>
    {{--                    </td>--}}
    {{--                    <td>--}}
